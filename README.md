...
内置模型（共 11 个，均支持函数调用与深度思考；模型名前缀为 `VolcengineCodingPlan/`，例如 `VolcengineCodingPlan/doubao-seed-2.1-turbo`）：
...
> `VisionModel` 必须是一个 capabilities 含 `vision` 的模型。火山方舟 Coding Plan 下的 `doubao-seed-2.1-turbo`、`doubao-seed-2.0-lite`、`doubao-seed-evolving`、`glm-5.3-flash`、`kimi-k2.7-code`、`kimi-k3` 等均支持视觉；`glm-5.3`、`deepseek-v4-*` 是纯文本模型，不能用作 `VisionModel`。
>
